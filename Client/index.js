...
async function saveExpense(e){
    ...
    if(amount&&description){
        ...
        if(mode==='newExpense'){
            const response=await axios.post('http://localhost:5100/postExpenses',expenses)
            const data=await response.data
            try {
                createExpense(data)
            } catch (error) {
            ...
            const response=await axios.put(`http://localhost:5100/updateExpenses/${mode}`,expenses)
            const data=await response.data
            try {
                createExpense(data)
                mode='newExpense'
...
function createExpense(details) {
    ...
    var info=document.createTextNode(`Rs.${details.amount}-->${details.description}-->${details.category}`);
    li.id=details.id
    li.appendChild(info)
...
async function deleteExpense(e) {
    ...
    const response=await axios.delete(`http://localhost:5100/deleteExpenses/${locate}`)
    try {
...
async function updateExpense(e){
    const locate=e.target.parentElement.id
    const response=await axios.get(`http://localhost:5100/updateExpenses/${locate}`)
    const data=await response.data
    try {
        expenselist.removeChild(e.target.parentElement)
